...
const ROLE_COLORS = {
  TeacherEnrollment: 'bg-blue-100 text-blue-800',
  TaEnrollment: 'bg-purple-100 text-purple-800',
  StudentEnrollment: 'bg-green-100 text-green-800',
  ObserverEnrollment: 'bg-gray-100 text-gray-800',
    ...
    return (
      <Layout>
        ...
        <div className="text-center py-12">
          ...
          <button onClick={fetchEnrollments} className="text-blue-600 hover:text-blue-800 text-sm font-medium">Try Again</button>
        </div>
      </Layout>
    );
  }
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-4">
          <form onSubmit={handleAddEnrollment} className="flex items-end gap-3">
            <div className="flex-1 relative">
              <label htmlFor="add-person-search" className="block text-sm font-medium text-gray-700 mb-1">Person</label>
              {selectedUser ? (
                <div className="flex items-center gap-2 border border-gray-300 rounded-md px-3 py-2 text-sm bg-gray-50">
                  ...
                  <span className="text-gray-900 font-medium truncate">{selectedUser.name}</span>
                  {selectedUser.email && <span className="text-gray-400 text-xs truncate">{selectedUser.email}</span>}
                  <button type="button" onClick={clearSelectedUser} className="ml-auto text-gray-400 hover:text-gray-600 flex-shrink-0">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
                        ...
                        userResults.map((u) => (
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              <div className="text-sm font-medium text-gray-900 truncate">{u.name || `User ${u.id}`}</div>
            ...
            <div>
              <label htmlFor="add-person-role" className="block text-sm font-medium text-gray-700 mb-1">Role</label>
              <select
                id="add-person-role"
                value={addForm.type}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-full ${filter === 'all' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
          >
          ...
          {Object.entries(roleCounts).map(([role, count]) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-full ${filter === role ? ROLE_COLORS[role] || 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
                  ...
                  <tr key={enrollment.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 text-sm font-medium flex-shrink-0">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${ROLE_COLORS[enrollment.type] || 'bg-gray-100 text-gray-800'}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        enrollment.enrollment_state === 'active' ? 'bg-green-100 text-green-800' :
                        enrollment.enrollment_state === 'invited' ? 'bg-yellow-100 text-yellow-800' :
